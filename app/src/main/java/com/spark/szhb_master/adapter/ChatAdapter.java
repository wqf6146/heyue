...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        LogUtils.i("onBindViewHolder()  myId  " + myId + "    data " + datas.get(position).getUidFrom());
        if (myId.equals(datas.get(position).getUidFrom())) {
            ...
            if (MyApplication.app.getCurrentUser().getAvatar() == null) {
                ...
                Glide.with(context).load(MyApplication.app.getCurrentUser().getAvatar())
                        .placeholder(R.mipmap.icon_default_header_grey).into(holder.ivHeaderRight);
            }
            ...
            Glide.with(context).load(datas.get(position).getFromAvatar())
                    .placeholder(R.mipmap.icon_default_header_grey).into(holder.ivHeaderLeft);
            holder.tvMessageLeft.setText(datas.get(position).getContent());
            //LogUtils.logi("ChatActivity","onBindViewHolder()"+holder.tvMessageRight.getText());
        }
    }

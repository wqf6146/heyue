 ...
 */
public class SuanActivity  extends BaseActivity{

    @BindView(R.id.ivBack)
    ImageView ivBack;@BindView(R.id.iv_spinner)
    ImageView iv_spinner; @BindView(R.id.image_frd)
    ImageView image_frd;@BindView(R.id.tvCopy)
    ImageView tvCopy;@BindView(R.id.tvSave)
    ImageView tvSave;@BindView(R.id.rela_frd)
    LinearLayout rela_frd;@BindView(R.id.ll_haibao)
    LinearLayout ll_haibao;
    @BindView(R.id.wx_frd)
    ImageView wx_frd;
    @BindView(R.id.wx_cal)
    ImageView wx_cal;@BindView(R.id.text_code)
    TextView text_code;
    private String url;
    ...
    protected void initImmersionBar() {
        ...
        if (!isSetTitle) {
            immersionBar.setTitleBar(SuanActivity.this, llTitle);
            isSetTitle = true;
        }
    }
    ...
    protected int getActivityLayoutId() {
        return R.layout.suan_frd_layout;
    }
    @Override
    protected void initData() {
        super.initData();
        tvGoto.setVisibility(View.INVISIBLE);
//        iv_spinner.setVisibility(View.INVISIBLE);
    ...
    protected void initView() {
        ...
        url = user.getPromotionPrefix() + user.getPromotionCode()+"&from=treasure";
        text_code.setText("邀请码："+user.getPromotionCode());
    }
    ...
    @OnClick({R.id.ivBack,R.id.tvCopy,R.id.tvSave,R.id.iv_spinner,R.id.wx_cal,R.id.wx_frd})
    @Override
    protected void setOnClickListener(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.iv_spinner:
                startActivity(new Intent(SuanActivity.this,SpinnerActivity.class).putExtra("url", user.getPromotionPrefix() + user.getPromotionCode()+"&from=treasure"));
                break;
            case R.id.wx_cal:
                wx_cal.setEnabled(false);
                ShareUtils.shareWeb(SuanActivity.this, url, GlobalConstant.share_title+"--"+ GlobalConstant.share_des, GlobalConstant.share_description,
                        GlobalConstant.logo_trea, R.mipmap.frd_wx, SHARE_MEDIA.WEIXIN_CIRCLE);
                break;
            case R.id.wx_frd:
                wx_frd.setEnabled(false);
                ShareUtils.shareWeb(SuanActivity.this, url,  GlobalConstant.share_title, GlobalConstant.share_description,
    ...
    private void checkPermission(int requestCode, String[] permissions) {
        AndPermission.with(SuanActivity.this).requestCode(requestCode).permission(permissions).callback(permissionListener).start();
    }

...
public class ChatListAdapter extends BaseQuickAdapter<ChatTable, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, ChatTable item) {
        ...
        Glide.with(context.getApplicationContext()).load(item.getFromAvatar()).placeholder(R.mipmap.icon_default_header_grey).into((CircleImageView) helper.getView(R.id.ivHeader));

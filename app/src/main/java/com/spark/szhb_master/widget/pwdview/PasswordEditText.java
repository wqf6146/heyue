...
public class PasswordEditText extends AppCompatEditText {
    ...
    // 密码圆点的颜色
    private int mPasswordColor = Color.parseColor("#ffffff");
    // 密码圆点的半径大小
    ...
    private void initPaint() {
        ...
        mRectStokePaint = new Paint();
        mRectStokePaint.setAntiAlias(true);
        mRectStokePaint.setDither(true);
        mRectStokePaint.setColor(mDivisionLineColor);
    ...
    private void drawDivisionLine(Canvas canvas) {
        mRectStokePaint.setStrokeWidth(mDivisionLineSize);
        for (int i = 0; i < mPasswordNumber - 1; i++) {
            int startX = mBgSize + (i + 1) * mPasswordItemWidth + i * mDivisionLineSize;
            int startY = 0;
            int endX = startX;
            int endY = getHeight();
            canvas.drawLine(startX, startY, endX, endY, mRectStokePaint);
        }
    }
